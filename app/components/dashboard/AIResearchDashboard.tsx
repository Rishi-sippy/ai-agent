...
export default function AIResearchDashboard() {
  ...
  const handleExportPDF = () => {
    ...
    messages.forEach((msg) => {
      const text = `${msg.role === 'user' ? 'User' : 'AI'}:\n${msg.content}\n\n`
  ...
  const handleSearch = () => {
    ...
    // ✅ Add BOTH user + empty AI message immediately
    ...
    const eventSource = new EventSource(`http://127.0.0.1:8000/research-stream?query=${encodeURIComponent(userQuery)}`)
    ...
    eventSource.onmessage = (event) => {
      ...
      // ✅ ALWAYS update last assistant message
      setMessages((prev) => {
        const updated = [...prev]
  ...
  return (
    <div className="flex h-screen bg-[#0B1120] text-white">
      ...
      <main className="flex-1 flex flex-col">
        ...
        <button onClick={handleExportPDF} className="px-4 py-2 rounded-xl bg-white/10 hover:bg-white/20 text-sm">
          Export PDF
        </button>
        {/* Chat */}
          ...
          {messages.map((msg, i) => {
            ...
            return (
              <div key={i} className="space-y-4">
                {/* Message bubble */}
                <div className={`max-w-3xl p-4 rounded-2xl ${msg.role === 'user' ? 'ml-auto bg-white text-black' : 'bg-white/5 border border-white/10'}`}>
                  {msg.role === 'assistant' ? msg.content ? <ReactMarkdown>{msg.content}</ReactMarkdown> : <span className="animate-pulse text-gray-400">Thinking...</span> : msg.content}
                </div>
        ...
        <div className="p-4 border-t border-white/10 flex gap-3">
          ...
          {isStreaming && <div className="text-sm text-gray-400 animate-pulse">⚡ Rishi's bot is thinking...</div>}
        </div>
      </main>
    </div>
  )
